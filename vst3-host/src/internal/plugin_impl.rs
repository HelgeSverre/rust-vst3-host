...
use std::ptr;
use std::sync::Arc;
use vst3::{ComPtr, ComWrapper, Interface, Steinberg::*, Steinberg::Vst::*};
...
use super::com_implementations::{ComponentHandler, HostEventList, ParameterChanges, create_event_list};
...
impl PluginImpl {
    ...
    pub fn load(path: &std::path::Path, info: PluginInfo) -> Result<Self> {
        unsafe {
            ...
            let get_factory: libloading::Symbol<GetPluginFactoryFunc> = library.get(b"GetPluginFactory\0")
                .map_err(|e| Error::PluginLoadFailed(format!("Failed to find GetPluginFactory: {}", e)))?;
            
            let factory_ptr = get_factory();
            if factory_ptr.is_null() {
                return Err(Error::PluginLoadFailed("GetPluginFactory returned null".to_string()));
            }
            
            let factory = ComPtr::<IPluginFactory>::from_raw(factory_ptr)
                .ok_or_else(|| Error::PluginLoadFailed("Failed to create factory ComPtr".to_string()))?;
            ...
            let processor = component.cast::<IAudioProcessor>()
                .ok_or_else(|| Error::InterfaceError("Component does not implement IAudioProcessor".to_string()))?;
            ...
            if factory.getClassInfo(i, &mut class_info) == kResultOk {
                ...
                if category.contains("Audio Module Class") {
                    ...
                    if result == kResultOk && !component_ptr.is_null() {
                        return ComPtr::from_raw(component_ptr)
                            .ok_or_else(|| Error::PluginLoadFailed("Failed to create component".to_string()));
                    }
                }
            }
        }
        
        Err(Error::PluginLoadFailed("No audio component found in plugin".to_string()))
    }
    ...
    fn setup_processing(&mut self) -> Result<()> {
        unsafe {
            ...
            let result = self.processor.setupProcessing(&setup);
            if result != kResultOk {
                return Err(Error::InterfaceError(format!("Failed to setup processing: {:#x}", result)));
            }
    ...
    fn create_process_data(&mut self) -> Result<()> {
        unsafe {
            ...
            // Set up event lists
            data.process_data.inputEvents = self.input_events.to_com_ptr::<IEventList>()
                .map(|ptr| ptr.into_raw())
                .unwrap_or(ptr::null_mut());
            data.process_data.outputEvents = self.output_events.to_com_ptr::<IEventList>()
                .map(|ptr| ptr.into_raw())
                .unwrap_or(ptr::null_mut());
            
            // Set up parameter changes
            data.process_data.inputParameterChanges = data.input_param_changes.to_com_ptr::<IParameterChanges>()
                .map(|ptr| ptr.into_raw())
                .unwrap_or(ptr::null_mut());
            data.process_data.outputParameterChanges = data.output_param_changes.to_com_ptr::<IParameterChanges>()
                .map(|ptr| ptr.into_raw())
                .unwrap_or(ptr::null_mut());
    ...
    unsafe fn prepare_buffers(&mut self, data: &mut HostProcessData) -> Result<()> {
        ...
        for bus_idx in 0..output_bus_count {
            ...
            if self.component.getBusInfo(kAudio as i32, kOutput as i32, bus_idx, &mut bus_info) == kResultOk {
                let channel_count = bus_info.channelCount;
                
                // Activate the bus
                self.component.activateBus(kAudio as i32, kOutput as i32, bus_idx, 1);
...
impl PluginInternal for PluginImpl {
    ...
    fn get_parameter(&self, id: u32) -> Result<f64> {
        if let Some(ref controller) = self.controller {
            unsafe {
                Ok(controller.getParamNormalized(id))
            }
        } else {
            Err(Error::InterfaceError("No controller available".to_string()))
        }
                ...
                for i in 0..count {
                    ...
                    if controller.getParameterInfo(i, &mut info) == kResultOk {
                        let param = Parameter {
                            ...
                            can_automate: (info.flags & ParameterInfo_::ParameterFlags_::kCanAutomate as i32) != 0,
                            is_read_only: (info.flags & ParameterInfo_::ParameterFlags_::kIsReadOnly as i32) != 0,
                            is_bypass: (info.flags & ParameterInfo_::ParameterFlags_::kIsBypass as i32) != 0,
    ...
    fn send_midi_event(&mut self, event: MidiEvent) -> Result<()> {
        ...
        let mut vst_event = Event {
            busIndex: 0,
            sampleOffset: 0,
            ppqPosition: 0.0,
            flags: Event_::EventFlags_::kIsLive as u16,
            __field0: Event___field0 { type_: 0 },
        };
        
        match event {
            MidiEvent::NoteOn { channel, note, velocity } => {
                vst_event.__field0.type_ = kNoteOnEvent as u16;
                unsafe {
                    vst_event.__field0.__field1.noteOn = NoteOnEvent {
                        channel: channel.as_index() as i16,
                        pitch: note as i16,
                        tuning: 0.0,
            ...
            MidiEvent::NoteOff { channel, note, velocity } => {
                vst_event.__field0.type_ = kNoteOffEvent as u16;
                unsafe {
                    vst_event.__field0.__field1.noteOff = NoteOffEvent {
                        channel: channel.as_index() as i16,
                        pitch: note as i16,
                        velocity: velocity as f32 / 127.0,
                        noteId: -1,
            ...
            _ => {
                ...
                return Err(Error::MidiError("MIDI event type not yet implemented".to_string()));
            }
        }
    ...
    fn start_processing(&mut self) -> Result<()> {
        unsafe {
            ...
            if result != kResultOk {
                return Err(Error::Other(format!("Failed to start processing: {:#x}", result)));
            }
    ...
    fn open_editor(&mut self, _parent: *mut std::ffi::c_void) -> Result<()> {
        ...
        Err(Error::Other("Editor support not yet implemented".to_string()))
    }
...
impl PluginImpl {
    ...
    fn send_legacy_midi_cc(&mut self, channel: MidiChannel, controller: u8, value: u8) -> Result<()> {
        use vst3::Steinberg::Vst::Event_::EventTypes_::*;
        ...
        let mut event = Event {
            ...
            ppqPosition: 0.0,
            flags: Event_::EventFlags_::kIsLive as u16,
            __field0: Event___field0 { type_: kLegacyMIDICCOutEvent as u16 },
        };
        
        unsafe {
            event.__field0.__field1.midiCCOut = LegacyMIDICCOutEvent {
